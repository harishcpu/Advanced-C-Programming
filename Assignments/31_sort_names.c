 ...
 * Author : Harish
 * Date : 21 Dec
 * Description : read N names, Sort the names in Alphabetical order, print them
 */
...
{
	int n;
	printf("Enter total "  BOLDRED "count of strings" RESET " to be stored : ");
	...
	printf("Enter the %d names " BOLDRED "(max 32 characters in each)\n" RESET, n);
	for (int i = 0; i < n; i++) {
    ...
    for (int i = 0; i < n; ++i) 		// print n names
		printf(BOLDRED "[" RESET "%d" BOLDRED "] -> " GREEN "%s\n" RESET, i+1, *(names + i));
}
